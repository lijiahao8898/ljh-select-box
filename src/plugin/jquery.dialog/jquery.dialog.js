...
var jDialog = (function($, undefined) {
    ...
    DialogClass.prototype = {
        ...
        height: function(height) {
            ...
            if (height) {
                ...
                switch (bodyPaddings.length) {
                    case 4:
                    case 3:
                        shellHeight += (parseInt(bodyPaddings[0], 10) || 0) + (parseInt(bodyPaddings[2], 10) || 0);
                        break;
                    case 2:
                    case 1:
        ...
        _init: function() {
            ...
            if (!isNaN(parseInt(this.cfg.top)) || !isNaN(parseInt(this.cfg.left)) || this.cfg.anchor) {
        ...
        _setupTriangle: function() {
            ...
            switch ((this.cfg.anchor.position || 'right').toLowerCase()) {
                ...
                case 'bottom':
                    posTriangle = {
                        top: -(domDialogSize.height + 12),
                        left: tpfs ? tpfs : (domDialogSize.width - 12) / 2
                    };
                    posDialog = {
                ...
                case 'left':
                    posTriangle = {
                        top: tpfs ? tpfs - domDialogSize.height : -(domDialogSize.height + 12) / 2,
                ...
                $.each(['es'], function(k, v) {
                    ...
                    jDrag.init({
                        ...
                        onMove: function(event, x, y) {
                            var width = val_W + x;
                            var height = val_H + y;
                            if (!((self.cfg.minWidth && width < self.cfg.minWidth && x < 0) || (self.cfg.maxWidth && (width > self.cfg.maxWidth) && x > 0)))
                                self.dom.element.width(width);
                            if (!((self.cfg.minHeight && (height < self.cfg.minHeight) && y < 0) || ((self.cfg.maxHeight) && (height > self.cfg.maxHeight) && y > 0)))
                                con.height(height);
